...
**Thinking level**:
用户可调的、驱动本轮 LLM 推理深度的强度选项:轻/中/高三档,常驻在发送按钮旁,全局持久化(与 provider/model 同层的偏好设置),不随 [Session] 走。只在意图分类判定为 compose 的 [Agent turn] 下生效,决定思考预算/effort 有多深;chat 意图下这轮完全不思考、也不展示思考链,与现状一致——Thinking level 不接管 compose/chat 这层开关,只接管"开了之后思考多深"。思考始终开启、不提供"关闭"档;"高"始终对应该型号真正能表达的最深一档(max / xhigh / 最大思考预算)。各 provider/型号实际能表达的档位有限时,UI 只展示该型号真正区分得开的档位(`getSupportedThinkingLevels()`),例如 glm-5.2 只展示"中/高"两档,deepseek-v4-flash 展示"轻/中/高"三档、deepseek-v4-pro 只展示"中/高",完全没有分级能力的型号(kimi、glm-5.1 系列、claude-haiku-4-5 / claude-sonnet-4-5)则不渲染该控件、不给选择。全局存储的偏好值不因型号切换而被覆盖——只在展示时按 `clampThinkingLevel()` 就近折算到当前型号支持的档位,切回支持完整档位的型号后偏好值原样生效。指针交互约定:打开入口、点击/拖动档位、关闭浮层都不改变输入框焦点、不主动控制软键盘——控件子树标记 `data-chat-input-focus-ignore` 排除在输入卡片"点空白聚焦"之外,输入框聚焦期间控件在按下阶段取消默认焦点转移并自行驱动滑杆手势;键盘可访问性(Tab、方向键、Escape)保持原生行为。光标约定:浮层可见期间暂时隐藏关联输入框的插入光标(caret-color: transparent,由 `.chat-input-surface:has([data-thinking-level-popover])` 按实际 DOM 派生,视频模式的内联透明规则不受影响),浮层移除(关闭、禁用、型号无档位、回放卸载)后自然恢复既有光标规则;指针操作始终不改变输入框焦点、文本或选区。
