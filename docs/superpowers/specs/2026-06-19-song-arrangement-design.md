...
2. **编排网格（入场/出场）**：教 agent 用 `.mask(...)` / 大尺度 `.gain(...)` 决定每层何时进入/退出，共用同一窗口、边界对齐，拉出 intro→build→main→breakdown→return 弧线。**窗口长度与段数由 agent 按风格/情绪自选，不规定默认值**。
...
5. **原则而非模板（措辞要求）**：编排作为一套可自由组合的原则呈现，不是可照抄的模板骨架；窗口长度、段数、入场顺序、门控 vs 渐变、用哪几种微观变化手段，都由 agent 按风格/情绪自选。

6. **提交前自检追加一条**（在"以音乐家的耳朵聆听"节）：「这是完整曲子吗？若是，各层是否随时间错开入场/出场拉出弧线、内部是否有混合周期/连续信号/随机带来的微观变化，而不是从头叠到尾？」
