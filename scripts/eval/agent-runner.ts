// scripts/eval/agent-runner.ts
// Agent 循环封装，支持单轮和多轮对话。
...
// ============================================================================
// 内联 parser.ts 的核心逻辑（完整复制自 scripts/run-agent-testcases.ts）
// ============================================================================
...
// ============================================================================
// 内联 tools.ts 的核心逻辑（完整复制自 scripts/run-agent-testcases.ts）
// ============================================================================
...
// ============================================================================
// System Prompts（完整复制自 scripts/run-agent-testcases.ts，并导出）
// ============================================================================
...
// ============================================================================
// Anthropic 消息格式转换（完整复制自 scripts/run-agent-testcases.ts）
// ============================================================================
...
// ============================================================================
// Helper: create improviseLLM closure with a given system prompt
// ============================================================================

function makeImprovise(improviseSystemPrompt: string) {
  return async (role: string, style: string, complementTask: string, hints: string, currentCode: string): Promise<string> => {
    ...
    try {
      const resp = await client.messages.create({
        model: ANTHROPIC_MODEL, system: improviseSystemPrompt,
...
export interface RunTurnOptions {
  instruction: string;
  currentCode: string;        // Previous turn's committed code. Empty string for turn 1.
  agentSystemPrompt: string;
  improviseSystemPrompt: string;
  maxIter?: number;
}
...
export async function runAgentTurn(opts: RunTurnOptions): Promise<RunTurnResult> {
  const t0 = Date.now();
  const state: AgentState = { code: opts.currentCode, finalCode: null };

  const localImprovise = makeImprovise(opts.improviseSystemPrompt);
  const ctx: ToolContext = { state, improviseLLM: localImprovise };

  const tools = getToolSchemas();
  const messages: ChatMsg[] = [
    { role: 'system', content: opts.agentSystemPrompt },
    { role: 'user', content: opts.currentCode
        ? `用户指令: ${opts.instruction}\n\n当前代码:\n${opts.currentCode}`
        : `用户指令: ${opts.instruction}` },
  ];

  const MAX_ITER = opts.maxIter ?? 30;
  let explanation = '';
    ...
    for (const tc of toolCalls) {
      ...
      try {
        ...
        if (e instanceof CommitSignal) {
          ...
          return { code: e.code, explanation, durationMs: Date.now() - t0 };
        }
  ...
  return { code: state.code || 'silence', explanation, durationMs: Date.now() - t0 };
}
