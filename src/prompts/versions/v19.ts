...
export const AGENT_SYSTEM_PROMPT_OPENAI = [
  ...
  [
    ...
    '行为样例（同样用来归纳，而非照抄）：反例——贝斯 `chords.n("0")` 让一个根音占满整个 cycle 当铺底，或 `<...>/4` 让一条线一个音横跨数 cycle "自我演化"，在慢速下就是十几秒长音、像在凑时长；正例——同样在场的这段时间，用带休止与根音走动的八分 ostinato（如 `chords.n("0 ~ 5 0 ~ 0 7 ~")`）或更短发音的动机反复重组去填满，"长句感"靠把许多短事件串成一句，而不是把一个事件拉长。',
...
export const AGENT_SYSTEM_PROMPT_EN = [
  ...
  [
    ...
    'Worked examples (again to induce, not to copy): counter-example — a bass `chords.n("0")` letting one root fill a whole cycle as bedding, or `<...>/4` letting one note of a line span several cycles "evolving on its own", which at slow tempo is a dozen-second sustain that sounds like padding the runtime; positive — over that same span of presence, fill with an eighth-note ostinato with rests and root motion (e.g. `chords.n("0 ~ 5 0 ~ 0 7 ~")`) or short-articulation motifs recombined, so the "long-phrase feel" comes from stringing many short events into a phrase rather than stretching one event.',
