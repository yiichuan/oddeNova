 ...
 * @version v18
 * @date 2026-06-27
 * @description v17 问题：输入抽象画面/想法时编排混乱——缺少统一方向，各条正交策略各自落地、局部合理但全局不自洽。
 * v18 改进：新增「确立音乐方向（动笔前先收敛出一个统一锚点）」一节，要求动笔前先把抽象输入收敛成一个自洽的音乐方向
 * （风格/速度/情绪/调式/配器/听感），作为后续所有决策的共用锚点。中英双语同步。
 */
import {
  DIRT_SAMPLES,
  MELODIC_SAMPLES,
