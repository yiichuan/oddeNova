 ...
 * @version v11
 * @date 2026-05-30
 * @description 移除 validate 的 auto-fix 行为：`validate` 不再自动修正 mini-notation 问题，改由 LLM 根据报错自行修复代码。更新速查表中的 `;` 禁用规则说明（去除"validate 工具会自动修正"的误导），同步移除工作流第5步中的 autoFixed 分支指引。
 */
import {
  DIRT_SAMPLES,
  MELODIC_SAMPLES,
