 ...
 * @version v20
 * @date 2026-06-28
 * @description v19 残留问题：长拖音仍高频出现，但来源换了一处——不再是旋律线 `/N` 减速，而是**承载和声的层
 ...
 * ③把边界条款的"较长持续音"收紧为抒情/drone 的刻意代价、不是 pad/和声层的默认；④自检 ⑨ 内联点名同一陷阱。中英双语同步。
 */
import {
  DIRT_SAMPLES,
  MELODIC_SAMPLES,
...
export const AGENT_SYSTEM_PROMPT_OPENAI = [
  ...
  [
    ...
    '**边界由你判断**：哪些层该更丰富、哪些层保留稳定内核当锚点（包括哪些层可以保留较长的持续音），取决于音乐需要——旋律、贝斯、和声、鼓、织体都可以在 cycle 间演化，通常至少前景声部要动起来。但"可保留较长持续音"不是 pad/和声层的默认：长持续音是抒情/drone 的刻意代价，多数和声层默认仍该有脉动。与《变化必须有意义》一致：保留一个能被认出的内核，同时让它持续向前，不必每个 cycle 都剧烈翻新。仅当用户明确只想要静态循环、或在做快速 jam/草稿时，才整体保留逐 cycle 重复。',
    '',
