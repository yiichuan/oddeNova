 ...
 * @version v20
 * @date 2026-06-19
 * @description 将人格块改为运行时注入，支持内置 oddeNova 与用户自定义人格切换。
 */
import {
  DIRT_SAMPLES,
  MELODIC_SAMPLES,
...
export function AGENT_SYSTEM_PROMPT_OPENAI(personaBlock: string, personaName: string): string {
  ...
  `- \`commit({ explanation })\` —— \`explanation\` 字段**必填**，用**中文**和 ${personaName} 的口吻撰写：像亲近的朋友一样温暖、自然、带一点星光感，但不要牺牲清晰度。请写两部分，用空行分隔：(1) 1句简短中文句子描述你为用户做了什么音乐变化（例如"我给这段心事铺了一层 lo-fi 鼓点和 808 贝斯，让它慢慢亮起来。"）；(2) 2条下一步建议，格式必须保持为"接下来可以：\n- [建议1（8-15 字）]\n- [建议2（8-15 字）]"。策略：≤1 个音层 → 建议添加缺失的鼓组/贝斯/旋律；≥2 个音层 → 建议变奏、情绪变化或与风格匹配的效果优化。示例："我给这段心事铺了一层 lo-fi 鼓点和 808 贝斯，让它慢慢亮起来。\n\n接下来可以：\n- 铺一段温暖的键盘旋律\n- 给鼓点加点 swing 懒散感"。该字段会作为聊天回复展示给用户。`,
  '- 不要在 `commit` 之后调用任何工具。',
