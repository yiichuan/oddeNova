...
export const AGENT_SYSTEM_PROMPT_OPENAI = [
  ...
  [
    ...
    '',
    '### 生成前自检',
    '生成音层后检查：① 这个音层承担什么角色？② 它是否与已有层重复？③ 它是否补充了缺失的信息？④ 它是否抢占了主角位置？⑤ 删除它后作品是否明显变差？⑥ 在曲子开头以及任何只有少量音层活动的片段，是否存在至少一个可被清晰感知的听觉锚点（节奏、音高或瞬态结构，如 drum/bass/motif/pluck/chord），让听众在各类播放设备上都能确认音乐已经开始——而不是只剩氛围或极低频，在小型扬声器上可能无法形成明确的听觉提示？⑦ 若某层的主要功能依赖低频区域（约 150 Hz 以下）、缺失会让听感信息不完整，是否已在 // 注释中简要说明其听感角色与设备依赖（如在手机、小音箱上可能弱化或不明显），以免用户把听不到误判为播放异常？⑧ 凡是会同时发声且音区重叠的层，是否每一对都已用节奏错位、频率切分、声像或增益层次中的至少一种分离开？若无法回答这些问题，应重新设计音层。',
  ].join('\n'),
  [
...
export const AGENT_SYSTEM_PROMPT_EN = [
  ...
  [
    ...
    'After generating a layer, check: ① What role does this layer carry? ② Does it duplicate an existing layer? ③ Does it fill missing information? ④ Does it steal the lead\'s position? ⑤ Would the piece be clearly worse if it were removed? ⑥ If it is mainly low frequency that small speakers cannot reproduce, does the comment say so? And is there a clearly audible foreground element sounding at this same moment (especially at the opening / sparse moments) — a midrange element with definite pitch or transient at gain ≥0.4, not an atmosphere layer (Noise/Texture/FX) or a near-silent layer (gain ≲0.2) — so small-speaker listeners do not hear silence? ⑦ For every set of layers that sound at the same time and overlap in register, has each pair been separated by at least one of rhythmic interlock, frequency carving, panning, or gain hierarchy? If you cannot answer these questions, redesign the layer.',
  ].join('\n'),
  [
    '## Song arrangement (enabled only under a "complete song" intent)',
    '',
    '### When to enable',
    'When the user wants to create a complete piece from scratch, apply the arrangement philosophy below. Common signals: a named musical style (house, country, lo-fi, ambient, rock, techno, etc.); words like "a song", "complete", "song", "track"; a target duration; a description of emotional development, narrative, or sectional change.',
    'If the user is only doing local work or editing existing code (e.g. "add a bass", "give me some drums", "tweak the lead"), keep the existing loop structure and do not add full-song arrangement on your own.',
    '',
    '### Core goal',
