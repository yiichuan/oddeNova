...
const STRUDEL_CHEATSHEET_CONCISE = [
  ...
  '- **让一条线演化、不逐 cycle 重复（按需使用）**：当某条线需要"长"起来时，把变化写进迷你记谱法本身。`<a b c>` 每 cycle 前进一步；**嵌套** `<a <b c>>` 时内层只在轮到它时才前进，于是整条线的真实周期是各层的最小公倍数——要十几个 cycle 才重复一次，听起来像被谱写的长旋律。例：旋律 `"[0 <4 3 <2 5>>*2](<3 5>,8)"`。节奏疏密同理：`*<2!3 4>`、欧拉 `(<3 5>,8)`。偶发装饰用 `.chunk(4, fast(2))`、`.sometimes(ply("2"))`、`.every(4, rev)`。注意：短循环（稳定的鼓底、riff、固定贝斯）本身完全成立，往往正是需要的；只在某条线确实该发展时才用嵌套，不要无差别套用。',
...
export const AGENT_SYSTEM_PROMPT_OPENAI = [
  ...
  [
    ...
    '原则同《变化必须有意义》：优先少数几个精心选择、缓慢演化的参数，而不是每个 cycle 都剧烈翻新；保留可辨识的内核，同时让它持续向前。不必每一层都演化——节奏与低频根基往往要稳，重点放在旋律、主奏与起装饰作用的声部上。',
    '',
    '### 低频可听性与感知补偿原则',
...
const STRUDEL_CHEATSHEET_EN = [
  ...
  '- **Make a line evolve instead of repeating every cycle (use when wanted)**: when a line should grow into a long phrase, bake the variation into the mini-notation itself. `<a b c>` advances one step per cycle; **nesting** `<a <b c>>` makes the inner step advance only when its turn comes, so the line\'s true period is the LCM of the layers — it takes a dozen-plus cycles to repeat and sounds like a composed long melody. E.g. melody `"[0 <4 3 <2 5>>*2](<3 5>,8)"`. Same for density: `*<2!3 4>`, Euclidean `(<3 5>,8)`. For occasional embellishment add `.chunk(4, fast(2))`, `.sometimes(ply("2"))`, `.every(4, rev)`. Note: a short loop (a steady drum bed, riff, or fixed bassline) is perfectly valid and often exactly what is wanted — reach for nesting only when a line genuinely should develop, not by default.',
