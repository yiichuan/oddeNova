...
const STRUDEL_CHEATSHEET_CONCISE = [
  ...
  '- 和声——让音符跟随移动的和弦进行，而非孤立音高：`chord("<Cm9 Fm9>/4").dict("ireal").voicing()` 生成并自动排布和弦；`.mode("root:g2")` 控制和弦排布方式、`.anchor("D5")` 固定音域；`n("0 1").set(chords)` 把音阶级数映射到当前和弦音，于是旋律/贝斯随和声演化。',
...
export const AGENT_SYSTEM_PROMPT_OPENAI = [
  ...
  [
    ...
    '- **用嵌套交替写出会演化的线条**：`<a b c>` 每个 cycle 前进一步；**嵌套** `<a <b c>>` 时，内层只在轮到它时才前进，于是整条线的真实周期是各层的最小公倍数，要十几个 cycle 才重复一次，听起来像被谱写的"长句"而非短循环。旋律例：`n("[0 <4 3 <2 5>>*2](<3 5>,8)").scale("D4:minor")`。节奏疏密同理：`struct` 用 `<...>`、密度用 `*<2!3 4>`、欧拉 `(<3 5>,8)`。**打击层同样适用，别只往旋律套**——hat 例 `n("[0 <1 3>]*<2!3 4>").s("hh")`：`*<2!3 4>` 让密度逐 cycle 呼吸（3 cycle 二连音、第 4 cycle 转四连音），`[0 <1 3>]` 交替采样位。',
...
const STRUDEL_CHEATSHEET_EN = [
  ...
  '- Harmony — let notes follow the moving chord progression instead of isolated pitches: `chord("<Cm9 Fm9>/4").dict("ireal").voicing()` builds and auto-voices the chords; `.mode("root:g2")` controls the voicing layout, `.anchor("D5")` fixes the register; `n("0 1").set(chords)` maps scale degrees onto the current chord tones so a melody/bass evolves with the harmony.',
