...
export const AGENT_SYSTEM_PROMPT_OPENAI = [
  ...
  [
    ...
    '- **算准入场时刻（两种写法都要会算）**：① 等分式 `<v1 … vk>/N`：每段 `N/k` cycle，第 m 段从 cycle `(m-1)·N/k` 起（如 `<0 1 1 1>/16` 在 cycle 4 入）。② 权重式 `<0@a 1@b 0@c>`（无斜杠，权重=各段 cycle 数）：某段从"前面权重之和"cycle 起（如 `<0@4 1@24 0@4>` 在 cycle 4 入、cycle 28 出）。**无论哪种写法，鼓 cycle 0、主旋律/lead 不晚于 cycle 8 入**——`<0@14 1@10 …>` 让主旋律 14 才进是错的；垫底的 pad/strings 可以更晚 swell 进来。',
    ...
    '- **❗不为变化而变化**：编排与变化都为"一首连贯的曲子"服务，宁可少而精准，不要给每层都撒满变化。',
    '- **微观变化要克制**：优先用**连续信号**（`sine/perlin` 调制 gain/lpf/pan）做细腻的呼吸感；和声/旋律可在不同长度的 `<...>` 间自然流动。**慎用改变速度/密度感的变换**——`.sometimes(fast(2))`、`.ply(N)`、`.chunk(N, fast(2))` 会让某些 cycle 的节奏突然变快/变密但**内容没变**，听感突兀像 glitch，尤其鼓组最忌；除非音乐上确实需要（如一处刻意的过门 fill），否则**不要给每层都贴**，且同一段落内速度与细分要保持一致。注意：不带 `/N`/`@` 的 `<a b>` 只是逐 cycle 交替，**不算**进出编排。',
    '- **要段落不同就换内容，不是变速**：让主歌/副歌听感不同，靠上一条「段落级内容变化」换鼓点/加 fill/换 voicing，**不要靠 `fast`/`ply` 把同一内容加速**——同内容变速最难听。',
    '- **原则而非模板**：窗口长度、段数、入场顺序、用门控还是渐变、用哪几种变化，都按当下风格情绪决定，不要套同一个固定骨架，也不要为凑"变化"而堆砌。',
  ].join('\n'),
].join('\n');
...
export const AGENT_SYSTEM_PROMPT_EN = [
  ...
  [
    ...
    '- **Compute entrance timing (know both notations)**: ① Equal-split `<v1 … vk>/N`: each segment is `N/k` cycles, segment m starts at cycle `(m-1)·N/k` (e.g. `<0 1 1 1>/16` enters at cycle 4). ② Weighted `<0@a 1@b 0@c>` (no slash; weights = cycles per step): a step starts at the sum of the preceding weights (e.g. `<0@4 1@24 0@4>` enters at cycle 4, exits at cycle 28). **In either notation, drums at cycle 0 and the lead/main melody no later than cycle 8** — `<0@14 1@10 …>` putting the melody in at cycle 14 is wrong; backing pads/strings may swell in later.',
    ...
    '- **❗Do not vary for the sake of varying**: arrangement and variation all serve one coherent song — prefer few and precise over sprinkling variation onto every layer.',
    '- **Keep micro-variation restrained**: prefer **continuous signals** (`sine/perlin` modulating gain/lpf/pan) for subtle breathing; harmony/melody may flow between `<...>` alternations of different lengths. **Be sparing with transforms that change the speed/density feel** — `.sometimes(fast(2))`, `.ply(N)`, `.chunk(N, fast(2))` make some cycles suddenly faster/busier while the **content is unchanged**, which sounds jarring and glitchy (worst on drums); unless the music genuinely needs it (e.g. a deliberate fill), **do not slap them on every layer**, and keep tempo/subdivision consistent within a section. Note: a bare `<a b>` without `/N` or `@` only alternates per cycle and does **not** count as entrance/exit arrangement.',
    '- **To make sections differ, change content, not speed**: differentiate verse/chorus via the per-section content change above (different beat, a fill, a new voicing) — **not** by speeding up the same content with `fast`/`ply`; speeding up unchanged content is the worst-sounding option.',
    '- **Principles, not a template**: window length, segment count, entrance order, gate vs. fade, and which variations to use are all chosen for the current style/mood; do not reuse one fixed skeleton, and do not pile on variation just to hit a quota.',
  ].join('\n'),
].join('\n');
