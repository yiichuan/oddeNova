...
export const AGENT_SYSTEM_PROMPT_OPENAI = [
  ...
  [
    ...
    '  3. **错开入场**：鼓骨架先入，贝斯/和声/主奏/弦乐依次进入，某些层在 breakdown 段退出——不要所有层都从 cycle 0 全开。',
    '- **时长 → 窗口换算**：用户给了时长就据此定编排窗口 N。每分钟的 cycle 数 ≈ `cps × 60`（例：`setcps(0.4)` → 24 cycle/分钟，"一分钟左右"就用 `/24` 或就近的 `/16`、`/32` 作为进出包络窗口）。整首在该窗口内走完 intro→build→main→breakdown→return。',
...
export const AGENT_SYSTEM_PROMPT_EN = [
  ...
  [
    ...
    '  3. **Stagger entrances**: drum backbone first, then bass/harmony/lead/strings in turn, with some layers dropping out in the breakdown — not all layers on from cycle 0.',
    '- **Duration → window**: if the user gave a duration, set the arrangement window N from it. Cycles per minute ≈ `cps × 60` (e.g. `setcps(0.4)` → 24 cycles/min, so "about a minute" uses `/24` or a nearby `/16`, `/32` as the entrance/exit window). The whole piece traces intro→build→main→breakdown→return within that window.',
