 ...
 * @version v3
 * @date 2026-05-22
 * @description 移除 improvise tool：将音乐层生成规则（频段互补、调性对齐、gain 范围、few-shot 示例）合并进主 agent 提示词；IMPROVISE_SYSTEM_PROMPT 标注 deprecated
 */
// ============================================================================
...
const REFERENCE_COMPOSITION = [
  '## Reference composition — "coastline" by eddyflux',
  'Study this as a gold-standard example of Strudel technique. It demonstrates: jazz chord/mode/voicing harmony, signal-modulated effects, 16-bar masking for arrangement, complex drum structs, and organic gain variation.',
  '```javascript',
  '// setcps(.75) → 180 BPM; use setTempo tool in practice',
  '// samples(\'github:eddyflux/crate\') → external bank; substitute .bank("RolandTR808") or gm_* names',
  ...
  '  chords.offset(-1).voicing().s("gm_epiano1:1") // CHORDS',
  '    .phaser(4).room(.5),',
  '  n("<0!3 1*2>").set(chords).mode("root:g2") // BASS',
  '    .voicing().s("gm_acoustic_bass"),',
  '  chords.n("[0 <4 3 <2 5>>*2](<3 5>,8)") // LEAD',
  '    .anchor("D5").voicing()',
  ...
  '- `chord(...).dict("ireal")` + `.voicing()` for jazz chord tones; `.offset(-1)` drops voicing register',
  '- `n(...).set(chords).mode("root:g2").voicing()` — bass locked to chord root at g2',
  '- `chords.n(...).anchor("D5").voicing()` — melodic patterns voiced around an anchor pitch',
  '- `.mask("<0 1 1 0>/16")` — 16-bar arrangement gate (intro silent, verse on, etc.)',
  '- `.segment(4).clip(rand.range(.4,.8))` — chop sustain into rhythmic fragments with random length',
  '- `.fm(sine.range(3,8).slow(8))` / `.lpf(sine.range(500,1000).slow(8))` — slow signal modulation for organic movement',
  '- `.rarely(ply("2")).chunk(4, fast(2))` — occasional doubling fills + periodic acceleration',
  '- `.gain(perlin.range(.6,.9))` — perlin noise for natural, breathing gain variation',
].join('\n');
...
export const AGENT_SYSTEM_PROMPT = [
  ...
  '5. After your last edit, run `validate` once on the final code. If it passes, `commit` directly.',
  '',
  '## Style matching',
  ...
  '- Match the user description to ONE of these by keyword (e.g. "学习/lo-fi/夜晚" → `lofi`, "快节奏/drum and bass" → `dnb`, "808/切分/drill" → `trap`, "爸士/swing/walking bass" → `jazz`). Use the matched style\'s BPM range as starting tempo and apply the style\'s characteristic timbres and techniques to your layer code.',
  '- If no style matches, fall back to your own judgment.',
  '',
  ...
  '8. **Style signatures (use the right technique for the style)**: each built-in style has a characteristic technique that defines its feel — apply it:\n  - lofi: hi-hat with `.late("[0 .01 0 .02]*4")` or `.swing(4)` for that lazy, drunk groove\n  - house: exactly `s("bd*4")` four-on-floor + euclidean hi-hat `(5,8)`; bass with `.gain("<.2 1@3>*2")` sidechain feel\n  - dnb: break pattern via `.struct("x ~ [x x] ~")` on snare; euclidean sub-fills\n  - ambient: `.room(0.8)` or higher + `.attack(≥1).release(≥2)` on pads; mandatory signal modulation on filter\n  - techno: `.mask("<1 1 1 0>/8")` for 8-bar tension/release cycle; lpf sweep via `sine.range`\n  - synthwave: arpeggiated lead with `.delay(0.5).delaytime(0.375)` for tempo-synced echo effect\n  - trap: 808 bass `.s("808bd").decay(0.6).sustain(0)` for pitch glide; hi-hat rolls `"hh*16"`\n  - jazz: `chord("<Am7 ...>").dict("ireal").voicing()` for real chord changes; euclidean or `.swing(4)` on drums',
  '',
  '## Iteration budget',
  ...
  '- 每次调用 `addLayer` 之前，思考文字只描述音乐意图（如"先铺一层鼓骨架"、"起手铺个底鼓"），不提工具名称和内部机制。',
  '',
  ...
  [
    ...
    '',
    '**Reference snippets** (adapt key/style to match actual context):',
    '```js',
    '// lofi drums',
    's("bd ~ sd ~")',
    '  .bank("RolandTR808")',
...
/**
 * @deprecated improvise tool 已移除（v3）。此提示词保留仅作历史参考，不再被任何代码路径调用。
 */
export const IMPROVISE_SYSTEM_PROMPT = [
  ...
  `- input: \`role: pad\\nstyle: ambient\` → ${JSON.stringify({ code: 'n("0 2 4 7").scale("C4:minor").s("sine").attack(0.1).release(0.5).gain(0.4)' })}`,
  '',
  'Rules:',
