 ...
 * @version v10
 * @date 2026-05-27
 * @description 新增 .add(s()) 禁用规则：明确禁止用 .add() 叠加采样音层（会触发 "cannot parse as numeral" 错误），应改用逗号语法或 stack()。
 */
import {
  DIRT_SAMPLES,
  MELODIC_SAMPLES,
...
const STRUDEL_CHEATSHEET_CONCISE = [
  '## Strudel 速查表（精简版）',
  '- 迷你记谱法：`*N` 重复，`/N` 减速，`[]` 分组，`<>` 交替循环，`,` 并行，`~` 休止，`(k,n)` 欧拉节奏，`!N` 复制，`@N` 延长。**禁止**使用 `_`（保持步长）——在 `,` 分支开头或 `[]` 内部会导致解析错误；请用显式值或 `@N` 代替。**禁止**在 `<>` 内使用 `|`——`|` 是随机选择运算符，在尖括号交替内无效；需要交替多步分组时，请写 `<[...] [...] [...]>`。**禁止**在 `<>` 内使用 `;`——`;` 不是合法迷你记谱法；在交替中表示同时和弦组，请写 `<[n1,n2,n3] [n4,n5,n6]>` 而不是 `<n1 n2 n3; n4 n5 n6>`（`validate` 工具会自动修正）。',
...
export const AGENT_SYSTEM_PROMPT_OPENAI = [
  ...
  '5. 最后一次编辑后，对最终代码运行一次 `validate`。若通过，直接 `commit`。若 `validate` 报告 `autoFixed`，代码已被自动修正——无需重新编辑，直接 `commit`。',
  ...
  '- 典型 3-4 音层创作：1 轮 `getScore`（若需要）+ 1 `setTempo` + 4×`addLayer` + 1 `validate` + 1 `commit` ≈ 7-8 轮。',
  '- **尽量批量操作**：单个助手轮次可以并行发出多个 `tool_calls`（例如同时 `addLayer drums` + `addLayer hh`）。利用此特性控制预算。',
  '',
  '## 音层命名',
  '- 使用语义化名称：`drums`、`hh`、`bass`、`pad`、`lead`、`fx`。代码库通过 `/* @layer NAME */` 注释保留这些名称——**不要**自己手写该注释，工具会处理。',
  '',
  ...
  '- 不要在 `commit` 之后调用任何工具。',
  '- **禁止**在任何地方写 `setcps(...)`——节奏由 `setTempo` 工具管理。',
  '- **禁止**在音层的 `code` 参数中包含外层 `stack(...)`——工具已经自动包裹。',
