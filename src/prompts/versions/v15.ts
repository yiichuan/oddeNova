 ...
 * @version v15
 * @date 2026-05-31
 * @description 在代码生成规范中增加注释要求：生成的 Strudel 代码必须在
 * setcps 前写顶部注释（`// STYLE | BPM: N`），以及在每个 @layer 标记后
 * 写一行简短的中文说明注释（乐器/音色/节奏意图）。
 */
import {
  DIRT_SAMPLES,
  MELODIC_SAMPLES,
...
export const AGENT_SYSTEM_PROMPT_OPENAI = [
  ...
  '- **代码注释**：生成的 Strudel 代码必须包含两种注释：(1) `setcps` 之前单独一行写顶部注释，格式 `// STYLE | BPM: N`（无匹配风格时写 `// BPM: N`）；(2) 每个 `/* @layer NAME */` 后另起一行，用 `// 中文简短说明`（10–20 字）描述本层乐器音色、节奏型或音乐意图。示例代码：\n  // lofi | BPM: 90\n  setcps(0.375)\n  stack(\n    /* @layer drums */\n    // tr808 鼓组，慵懒 swing 节奏骨架\n    s("bd sd hh")\n      .bank("RolandTR808")\n      .gain(0.8),\n    /* @layer bass */\n    // C 小调贝斯线，lpf 限制在低频段\n    note("c2 e2 g2")\n      .s("sawtooth")\n      .lpf(200)\n      .gain(0.7)\n  )',
  [
    '## 音层代码生成',
    '',
