...
export default function OddeNovaBridgeNotice({ status }: { status: OddeNovaBridgeStatus }) {
  ...
  let error = false;
  if (status.status === 'queued') copy = `第 ${status.revision} 版已排队，当前操作完成后应用`;
  else if (status.status === 'applied') {
    if (status.outcome === 'branched') {
      copy = `网页修改已保留，第 ${status.revision} 版已导入为新分支${status.persistent ? '' : '，但当前仅保存在内存'}`;
    } else if (!status.persistent) {
      copy = '页面已更新，但当前仅保存在内存';
    }
  } else if (status.status === 'recovery-required') { copy = status.message; error = true; }
  else if (status.status === 'disconnected') { copy = status.message ?? '本机连接已停止，请显式重新连接'; error = true; }
