...
/**
 * Bakes every (blur level, glyph) pair into one tiled canvas — glyphs across,
 * blur levels down.
 *
 ...
 * is tiny though — 24 glyphs at 8 radii — so the whole animation's blur can be
 ...
 */
function buildAtlas(pixelRatio: number): SpriteAtlas {
  ...
  const cellWidth = Math.ceil(PARTICLE_FONT_SIZE * 1.2) + padding * 2;
  const cellHeight = Math.ceil(PARTICLE_FONT_SIZE * 1.5) + padding * 2;
  ...
  PARTICLE_BLUR_LEVELS.forEach((blur, blurIndex) => {
    ...
    PARTICLE_GLYPHS.forEach((glyph, glyphIndex) => {
      context.fillText(
        glyph,
        glyphIndex * cellWidth + cellWidth / 2,
        blurIndex * cellHeight + cellHeight / 2,
        advance,
      );
    });
  });
...
/**
 * ASCII characters falling through the control bar like snow, standing in for
 * the visualizer while it is collapsed.
 */
export default function ControlBarParticles({
  ...
  useEffect(() => {
    ...
    let atlas = buildAtlas(Math.min(window.devicePixelRatio || 1, 2));
    ...
    const resize = () => {
      ...
      context.setTransform(pixelRatio, 0, 0, pixelRatio, 0, 0);
      if (atlas.pixelRatio !== pixelRatio) atlas = buildAtlas(pixelRatio);
    };
    ...
    const draw = (
      ...
      for (const particle of particles) {
        const flake = particleFrameAt(particle, timeSeconds, field, transition);
        ...
        const accent = particleAccentAt(particle, beatPosition) * accentMix;
    ...
    const step = (timeMs: number) => {
      ...
      draw(timeMs / 1000, transition, beatPositionAt(timeMs), accentMix);
