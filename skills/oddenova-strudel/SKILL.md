...
The first turn starts the local helper service and opens one short pairing entry. The page may ask for local-network permission. Later turns for the same project are queued for that paired page and do not open or focus another tab. A successful browser-launch command is not proof that the page applied the revision; only the helper's acknowledged status is.
...
Never autoplay. Tell the user to use oddeNova's play control after the page applies the version. Never paste connection credentials, a full import URL, or an encoded blob into chat. Do not create project files, call `/api/share`, or run oddeNova's internal agent loop.
