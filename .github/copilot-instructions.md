...
- **语言**：注释使用中文，外部 API 名称、协议关键字、错误信息及已有英文术语除外
- **必要性**：以下场景必须添加注释：
  - 非显而易见的业务逻辑或算法（说明「为什么」，而非复述代码在做什么）
  - 公共函数 / Hook / 类的用途与参数语义
  - 有意为之的「反直觉」写法（例如刻意绕过某个框架行为）
  - TODO / FIXME / HACK，需附上原因或 issue 链接
- **禁止**：不写无意义的冗余注释（如 `// 加一` 对应 `i++`）
...
**判断原则：** 若包内有 `let xxx; export function setXxx(v) { xxx = v; }` 这类 setter，从子路径导入就会破坏模块单例，一律只从包根导入。`@strudel/*` 系列包同理。需要包内部未导出的符号时，优先提 PR 请上游导出，而不是绕过去深引。

### WAV 导出时切换 AudioContext 的正确姿势
...
> `superdough` 的 `dist/index.mjs` 持有模块级单例。从子路径导入会载入独立模块图，其中的 `audioContext` 变量不会被 bundled setter 更新，最终导致 `InvalidAccessError: cannot connect to an AudioNode belonging to a different audio context`。症状：导出 WAV **缺失 delay/room 效果**，与实时播放听到的声音不一致。
